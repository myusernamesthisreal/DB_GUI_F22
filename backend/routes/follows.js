  ...
  app.get("/users/following",
    ...
    async (req, res) => {
      try {
        ...
        const followers = await query("SELECT users.username, users.displayname, users.id FROM db.follows JOIN users ON users.id = follows.src WHERE dst = ?", [src.id]);
        res.status(200).send({
          message: "Followers",
          success: true,
          followers,
  ...
  app.delete("/users/:id/unfollow",
    ...
    async (req, res) => {
      try {
        const src = await jwt.verifyToken(req);
        const { id: dstid } = req.params;
        const queryResult = await query("SELECT dst FROM db.follows WHERE id = ?", [dstid]);
        if (queryResult.length === 0)
          throw new Error("User not found");
        if (queryResult[0].id === src.id)
